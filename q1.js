...
const type = "grass";
const answer1 = pokes
  .filter(poke => Array.isArray(poke.types) && poke.types.includes(type))
  .map(poke => poke.name);
...
const answer2Or = pokes
  .filter(poke => Array.isArray(poke.types) && types.some(type => poke.types.includes(type)))
  .map(poke => poke.name);

const answer2And = pokes
  .filter(poke => Array.isArray(poke.types) && types.every(type => poke.types.includes(type)))
  .map(poke => poke.name);
